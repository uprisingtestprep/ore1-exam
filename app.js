/* ORE1 Exam Simulator, Application Logic */

const ACCESS_CODE  = "ORE19000";
const EXAM_SECONDS = 21600;
const PASSING_PCT  = 72;
const STORAGE_KEY  = "ore1_exam_state_v1";
const SIM_Q_COUNT  = 200;  // simulator serves this project's configured pool size (config.json exam.sim_questions)
const CLUSTER_LABEL = "EMQ Set";
const DOMAIN_LABELS = {"anatomy": "Anatomy", "physiology_and_biochemistry": "Physiology and Biochemistry", "oral_pathology_and_human_disease": "Oral Pathology and Human Disease", "microbiology_and_immunology": "Microbiology and Immunology", "pharmacology_and_therapeutics": "Pharmacology and Therapeutics", "dental_materials_science": "Dental Materials Science", "restorative_dentistry_and_endodontics": "Restorative Dentistry and Endodontics", "periodontology_prosthodontics_and_oral_surgery": "Periodontology Prosthodontics and Oral Surgery", "orthodontics_paediatric_dentistry_and_radiology": "Orthodontics Paediatric Dentistry and Radiology", "law_ethics_professionalism_and_health_and_safety": "Law Ethics Professionalism and Health and Safety"};  // maps domain key -> human-readable label for display
...
function renderQuestion() {
  ...
  // Derive the letter list from this question's own options, never a
  // hardcoded A-E. EMQ shares a 5-10 lettered list (up to J) per cluster;
  // a fixed list here would silently drop every letter past E, the same
  // class of bug documented in feedback_simulator_option_letter_hardcode.
...
function showResults() {
  ...
  document.getElementById("res-status").textContent = passed ? "ABOVE PRACTICE BENCHMARK" : "BELOW PRACTICE BENCHMARK";
...
function renderReview() {
  ...
  // Same fix as renderQuestion: derive letters from this question's own
  // options rather than a hardcoded A-E, so an EMQ answer up to J still
  // renders correctly in the review screen.
  Object.keys(q.options || {}).sort().forEach(letter => {
    const text = q.options?.[letter];
    if (!text) return;
    const div = document.createElement("div");
...
function keyHandler(e) {
  ...
  const q = questions[state.current - 1];
  // Accept any letter that is actually one of this question's own options
  // instead of a hardcoded A-E, so EMQ letters F through J work too.
  if (q?.options?.[letter] && !e.ctrlKey && !e.metaKey) {
